...
class Unit:
    ...
    def __mul__(self, __o: Union[Unit, ValidMagnitude]) -> Union[Unit, Quantity]:
        ...
        if isinstance(__o, Unit):
            ...
            # Dimensionless unit doesn't do anything
            if __o.unit_type == self.dimensionless_unit.unit_type:
                return self

            # If we are dimensionless, retain the unit of other class
            if self.dimensionless_unit == self:
                return __o
        ...
        if isinstance(__o, Quantity):
            return Quantity(__o.magnitude, __o.unit * self)
    ...
    def __truediv__(self, __o: Unit) -> UnitRatio:
        ...
        # TODO: handle dimensionless division of unit types.
...
class UnitRatio(Unit):
    ...
    def simplify(self) -> Tuple[float, Unit]:
        ...
        units_in_numerator: List[Unit] = self.numerator_unit.sorted_units if isinstance(self.numerator_unit, UnitProduct) else [self.numerator_unit]
        units_in_denominator: List[Unit] = self.denominator_unit.sorted_units if isinstance(self.denominator_unit, UnitProduct) else [self.denominator_unit]
        new_denominator = []
        ...
        if len(new_denominator) == 1:
            if len(units_in_numerator) == 0:
                return conversion_factor, UnitRatio(self.numerator_unit.dimensionless_unit, new_denominator[0])
            if len(units_in_numerator) == 1:
        ...
        if len(units_in_numerator) == 0:
            return conversion_factor, UnitRatio(self.numerator_unit.dimensionless_unit, UnitProduct(new_denominator))
        if len(units_in_numerator) == 1:
